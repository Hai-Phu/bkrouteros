...
define Device/hcmut_bkrouter-base
  KERNEL_INSTALL := 1
  KERNEL := kernel-bin
  DEVICE_TITLE := HCMUT BKRouter Base
  DEVICE_PACKAGES :=  \
  ...
  IMAGES := sdcard.img.gz
  IMAGE/sdcard.img.gz := boot-scr | boot-img-ext4 | sdcard-img-ext4 | gzip | append-metadata
  DEVICE_DTS := armada-388-bkrouter-base
  SUPPORTED_DEVICES += armada-388-bkrouter-base
  UBOOT := bkrouter-u-boot-spl.kwb
  BOOT_SCRIPT := bkrouter
endef
TARGET_DEVICES += hcmut_bkrouter-base

define Device/hcmut_bkrouter-base-v2
  KERNEL_INSTALL := 1
  KERNEL := kernel-bin
  DEVICE_TITLE := HCMUT BKRouter Base V2
  DEVICE_PACKAGES :=  \
    mkf2fs e2fsprogs kmod-fs-vfat kmod-nls-cp437 kmod-nls-iso8859-1 \
    wpad-basic kmod-ath9k kmod-ath10k-ct ath10k-firmware-qca988x-ct \
    partx-utils kmod-i2c-core kmod-i2c-mux kmod-i2c-mux-pca954x swconfig
  IMAGES := sdcard.img.gz
  IMAGE/sdcard.img.gz := boot-scr | boot-img-ext4 | sdcard-img-ext4 | gzip | append-metadata
  DEVICE_DTS := armada-388-bkrouter-base-v2
  SUPPORTED_DEVICES += armada-388-bkrouter-base-v2
  UBOOT := bkrouter-v2-u-boot-spl.kwb
  BOOT_SCRIPT := bkrouter
endef
TARGET_DEVICES += hcmut_bkrouter-base-v2
